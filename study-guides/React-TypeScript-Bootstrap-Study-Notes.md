...
| **8.6** | swap the rules for a schema | what zod replaces |
| **8.7** | look at uncontrolled inputs | the design the next step is built on |
| **8.8** | hand state to react-hook-form | `register`, `formState`, no re-render per keystroke |
| **8.9** | add `zodResolver` | the two axes are independent |
| **8.10** | read every input type raw | `checked`, `valueAsNumber`, `files`, `selectedOptions` |
| **8.11** | wrap all eleven types | one contract, eleven components |
| **8.12** | drive them with react-hook-form | `Controller`, and why our contract fits it |
...
**Don't skip to 8.8.** react-hook-form is worth using and it's only legible once you've felt what it removes. When it does something surprising at 5pm, steps 8.3–8.5 are the mental model you'll debug it with.
...
|---|---|---|---|
| `TextField.tsx` | 8.2 | 8.3 · 8.5 | The reusable text field. Superseded by `fields.tsx` in 8.11. |
| `Signup.tsx` | 8.2 | 8.3 · 8.4 · 8.5 | **The form.** Grows from inert markup to fully validated. |
| `signupValidation.ts` | 8.5 | — | The type, the empty value, and the pure `validateSignUp`. |
| `signupSchema.ts` | 8.6 | — | The zod schema, and the type inferred from it. |
| `SignupZod.tsx` | 8.6 | — | `Signup.tsx` with the schema instead of the validator. |
| `SignupRHF.tsx` | 8.8 | — | react-hook-form, rules inside `register`. |
| `SignupRHFZod.tsx` | 8.9 | — | react-hook-form + `zodResolver`. |
| `fields.tsx` | 8.11 | — | Eleven field components on one contract. |
| `fullSignupSchema.ts` | 8.12 | — | The 15-field schema for the everything-form. |

Steps 8.6, 8.8 and 8.9 create **new files rather than editing `Signup.tsx`**, deliberately: at the end you want all four versions side by side to compare, and §8.13 does exactly that.
...
## 8.6 Schema validation with zod

`validateSignUp` works, and it duplicates something you already declared. Your `SignUpValues` interface says `username: string`; your validator says "a non-empty string that looks like an email". Two descriptions of one thing, free to drift apart.

A **schema** collapses them into one:
...
export const SignUpSchema = z.object({
  ...
  username: z.string().min(1, "Email is required.").email("That doesn't look like an email address."),
  password: z
    .string()
...
/** The type is DERIVED from the schema — this replaces the hand-written interface. */
...
if (!result.success) {
  // { username: ["That doesn't look like an email address."], password: [...] }
  const fieldErrors = result.error.flatten().fieldErrors
} else {
  // result.data is typed SignUpValues, validated, and coerced
}
```

Five things this gets you that a hand-written validator doesn't:

1. **The type comes from the schema** (`z.infer`), so validation and types can't disagree. Add a field to the schema and TypeScript reports every place that needs it.
2. **`safeParse` returns a discriminated union** (`{ success: true, data }` or `{ success: false, error }`), so handling failure is narrowing (§5.3) rather than exception handling.
3. **Coercion.** `z.coerce.number()` turns the string `"42"` from a number input into `42` — solving §8.10's most common bug once, at the boundary, instead of at every input.
4. **Cross-field rules are first-class.** `.refine()` with a `path` decides which field shows the message.
5. **The same schema validates API responses** (§17.5) and `localStorage` (Build Step 7). One definition of "a valid sign-up", used everywhere data enters your program.

`flatten()` is the piece that makes this ergonomic in a form: it turns zod's nested issue list into `{ fieldErrors: Record<string, string[]>, formErrors: string[] }`, which maps straight onto per-field messages.

> **Version note.** These examples use **zod 3**, which is what most projects are on. Zod 4 moves some string formats to top-level functions — `z.string().email()` becomes `z.email()` — and `flatten()` is superseded by `z.treeifyError()`. The concepts are identical; check [zod.dev](https://zod.dev) for the version you install.

**What zod does *not* change:** the `useState`, the `update` helper, the `touched` map, `errorFor`, and every line of JSX. That's the point of the next lab — it changes one function and nothing else, which proves validation and state management are separate decisions.

---

## 🧪 Lab 8.6 — Swap the validator for a schema

**Level:** core

A **new file**, not an edit — you want both versions at the end to compare (§8.13).
...
**1. Create `src/demos/08-forms/signupSchema.ts`:**
...
/** Derived from the schema — replaces the hand-written interface. */
export type SignUpValues = z.infer<typeof SignUpSchema>
...
**2. Create `src/demos/08-forms/SignupZod.tsx`** — copy `Signup.tsx` and change the three marked lines:
...
import TextField from "@/demos/08-forms/TextField"
import {
  SignUpSchema,                                    // ← CHANGED: schema, not validator
  emptySignUp,
  type SignUpValues,
} from "@/demos/08-forms/signupSchema"

type Touched = Partial<Record<keyof SignUpValues, boolean>>
/** zod's flatten() reports an ARRAY of messages per field. */
...
function SignupZod() {
  ...
  // ← CHANGED: safeParse replaces validateSignUp(). Still derived every render.
  const result = SignUpSchema.safeParse(values)
  ...
  function errorFor(field: keyof SignUpValues): string | undefined {
    ...
    return errors[field]?.[0]                      // ← CHANGED: [0], because it's an array
  }
  ...
  function handleRegisterSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // parsed.data is typed, validated AND coerced — not the raw `values`
    console.info("✅ Valid — submitting", parsed.data)
  ...
  return (
    <>
      ...
      <Form onSubmit={handleRegisterSubmit} noValidate>
        {/* Every TextField below is IDENTICAL to Signup.tsx */}
        <TextField
          controlId="signUpZod.fname"
          label="First Name"
          value={values.firstName}
          onChange={(v) => update("firstName", v)}
          onBlur={() => touch("firstName")}
...
Create `src/demos/08-forms/SignupZodLab.tsx` — the same shape as `SignupLab`, rendering `<SignupZod />`:

```tsx
import { Card } from "react-bootstrap"
import DemoCard from "@/lab/DemoCard"
import SignupZod from "@/demos/08-forms/SignupZod"

export default function SignupZodLab() {
  return (
    <DemoCard
      title="Signup with zod"
      claim="Three lines changed from Signup.tsx. The state, the touched map and every field are identical — zod is a validation decision, not an architectural one."
      level="core"
      notice={
        <ul className="mb-0">
          <li>
            Behaviour is <strong>indistinguishable</strong> from <code>#signup</code>. That's
            the finding: the rules moved, nothing else did.
          </li>
          <li>
            Submit the empty form with the console open — <code>console.table</code> now
            prints zod's <code>fieldErrors</code>, an <em>array</em> of messages per field.
          </li>
          <li>
            <code>SignUpValues</code> is no longer declared anywhere. It's{" "}
            <code>z.infer&lt;typeof SignUpSchema&gt;</code>.
          </li>
        </ul>
      }
    >
...
```tsx
{ id: "signup-zod", chapter: "8 — Forms", title: "Signup with zod", element: <SignupZodLab /> },
```

### What to notice

**Three lines changed.** Here is the complete diff against `Signup.tsx`:

| | `Signup.tsx` | `SignupZod.tsx` |
|---|---|---|
| Type | `interface SignUpValues` in `signupValidation.ts` | `z.infer<typeof SignUpSchema>` |
| Rules | `validateSignUp(values)` → `{ field: message }` | `SignUpSchema.safeParse(values)` |
| Error shape | `string \| undefined` per field | `string[]` per field, via `flatten()` |
| Submitted value | `values` | `parsed.data` (validated, coerced) |

Everything else — `useState`, `update`, `touched`, `errorFor`, the guard clause, all four `TextField` call sites — is byte-for-byte identical. **That's why this lab exists:** it proves zod is a validation decision, not an architectural one.

Two details worth pausing on:

- **`flatten().fieldErrors` gives arrays.** A field can fail several rules at once, so zod reports a list. `errors[field]?.[0]` shows the first; swap it for a `<ul>` to show all.
- **Submit `parsed.data`, not `values`.** They're the same here, but the moment a schema does anything transformative — `z.coerce.number()`, `.trim()`, `.toLowerCase()`, a `.transform()` — only `parsed.data` has the converted values. The habit costs nothing and prevents a real bug.

### Experiments

1. **Add `confirmPassword` with a cross-field rule:**
   ```ts
   ...
   ```
   Compare with §8.5 experiment 5, where the same rule was one `if`. Both are easy — remember that when §8.8 makes it hard.
2. **Delete the `z.infer` line** and hand-write the interface instead. Then change a field in the schema and watch the two drift apart silently. `z.infer` is the whole "one source of truth" claim.
3. **Break the schema at runtime:** `SignUpSchema.safeParse({ firstName: 42 })` in the console. You get a structured error naming the field and the problem. Now do the equivalent with `validateSignUp` — it would happily call `.trim()` on a number and crash.
4. **Import the schema in a `.test.ts`** and assert on `safeParse` results. Same testability as the pure function, with less code.

---

## 8.7 Controlled vs uncontrolled
...
**This is the design react-hook-form is built on.** It keeps values in the DOM and reads them on submit, which is why §8.8's form doesn't re-render as you type. Read the next lab with this table in mind.

## 🧪 Lab 8.7 — Controlled vs uncontrolled
...
At some point the plumbing outweighs the learning value. Steps 8.4 and 8.5 wrote `values`, `update`, `touched`, `submitAttempted`, `errorFor`, and a guard clause — and every form you ever write needs the same six things. A form library owns them.
...
| Piece | Replaces, from steps 8.4–8.5 |
|---|---|
| `register("f", rules)` | `value` + `onChange` + `onBlur` + that field's rule |
| `handleSubmit(ok, fail)` | `e.preventDefault()`, `validateSignUp()`, the `if`/`return` guard |
| `formState.errors` | your derived `errors` object |
| `formState.touchedFields` | your `touched` map |
| `formState.isSubmitting` | a `submitting` boolean you'd have added |
| `formState.isDirty` | your `JSON.stringify` comparison |
| `reset()` | `setValues(emptySignUp)` plus clearing touched and submitAttempted |
| `setError` | a server-error branch you'd have hand-rolled |
| `mode: "onTouched"` | your `errorFor` strategy |

`handleSubmit(onValid, onInvalid)` takes **two** callbacks, which maps exactly onto the two exits you wrote by hand.

### Three things worth understanding, not just using

**1. It's uncontrolled by default, and that's the point.** `register` gives the input a `ref` and lets the DOM hold the value, so **typing doesn't re-render your component** — §8.7's right-hand column. The trade is that values aren't available during render unless you ask via `watch`, which re-subscribes that component to changes.

**2. `mode` is §8.5's "when do I show errors?" as one option.**

| `mode` | Validates |
|---|---|
| `"onSubmit"` (default) | Only on submit |
| `"onBlur"` | When a field loses focus |
| `"onTouched"` | On first blur, then on every change — §8.5's recommended strategy |
| `"onChange"` | Every keystroke |
| `"all"` | Blur and change |

**3. `{...register(...)}` spreads onto `Form.Control` but not onto our `TextField`.** React-bootstrap forwards refs (§15.5), so the spread reaches the real `<input>`. `TextField` takes `value`/`onChange` and accepts no ref, so `register` can't drive it — which is why this step writes `Form.Group` blocks by hand, and why §8.12 exists.

### The one thing it makes harder

Rules live inside `register` calls, scattered through the JSX. For simple rules that's readable. For a rule about *two* fields it isn't:

```tsx
{...register("confirmPassword", {
  validate: (v) => v === getValues("password") || "The two passwords don't match.",
})}
```

That reaches sideways with `getValues`, and it introduces a subtle bug: the rule only runs when **`confirmPassword`** is validated, so typing a new *password* afterwards leaves a stale result. The fix is `deps` on the other field:

```tsx
{...register("password", { /* … */ deps: ["confirmPassword"] })}
```

It works, and notice what it cost: a rule that logically belongs to *the form* is now spread across two fields, one of which knows the other's name twice. Compare with §8.6's one-line `.refine()`. That's §8.9.

---

## 🧪 Lab 8.8 — SignUp with react-hook-form
...
Create `src/demos/08-forms/SignupRHF.tsx`:
...
function SignupRHF() {
  ...
  async function onValid(values: SignUpValues) {
    await new Promise((r) => setTimeout(r, 700))    // stand-in for the API call

    if (values.username.endsWith("@taken.com")) {
      setError("username", { message: "That email is already registered." })
      return
    }
  ...
  function onInvalid(formErrors: FieldErrors<SignUpValues>) {
  ...
  return (
    <>
      ...
      <Form onSubmit={handleSubmit(onValid, onInvalid)} noValidate>
        <Form.Group className="mb-3" controlId="signUpRHF.fname">
        ...
        <Form.Group className="mb-3" controlId="signUpRHF.lname">
        ...
        <Form.Group className="mb-3" controlId="signUpRHF.username">
        ...
        <Form.Group className="mb-3" controlId="signUpRHF.password">
          ...
          <Form.Control
            ...
            {...register("password", {
              ...
              // `validate` is the escape hatch for rules the built-ins can't express
          ...
          <Form.Text>At least 8 characters, including a number.</Form.Text>
        </Form.Group>
...
Create `src/demos/08-forms/SignupRHFLab.tsx` on the same pattern, but add a `RenderBadge` — this lab's headline is a number:

```tsx
import { Card, Col, Row } from "react-bootstrap"
import DemoCard from "@/lab/DemoCard"
import RenderBadge from "@/lab/RenderBadge"
import SignupRHF from "@/demos/08-forms/SignupRHF"

export default function SignupRHFLab() {
  return (
    <DemoCard
      title="Signup with react-hook-form"
      claim="register replaces value/onChange/onBlur/touched; handleSubmit replaces the guard. And because the inputs are uncontrolled, typing no longer re-renders the form."
      level="core"
      notice={
        <ul className="mb-0">
          <li>
            <strong>Type in any field and watch the badge. It doesn't move.</strong> Compare
            with <code>#signup</code>, which re-renders on every keystroke.
          </li>
          <li>
            Gone from the component: <code>values</code>, <code>update</code>,{" "}
            <code>touched</code>, <code>submitAttempted</code>, <code>errorFor</code>, and
            the derived <code>errors</code>.
          </li>
          <li>
            Also gone: <code>TextField</code>. <code>register</code> spreads a ref and a
            DOM-shaped <code>onChange</code>, which it can't accept — so the markup is back
            to a <code>Form.Group</code> per field. §8.12 fixes that.
          </li>
          <li>
            Submit with an email ending <code>@taken.com</code> to see{" "}
            <code>setError</code> place a server-side failure on a field.
          </li>
        </ul>
      }
    >
...
Register as `{ id: "signup-rhf", chapter: "8 — Forms", title: "Signup with react-hook-form", element: <SignupRHFLab /> }`.

> The badge sits in the *wrapper*, so it counts the wrapper's renders. To count `SignupRHF`'s own, drop a `<RenderBadge label="inner" />` just inside its returned fragment.

### What to notice

- **The render count doesn't move as you type.** That's the whole design: `register` gives the input a `ref` and the DOM keeps the value.
- **`SignUpValues` still comes from `signupValidation.ts`.** The type is reused; only the rules moved.
- **Rules are now inside the JSX**, one blob per field. Readable at four fields.
- **`password` already needs `validate`**, because "must contain a digit" has no built-in. That's the first crack.
- **`isSubmitting` drives the spinner** with no boolean of your own.

### Experiments

1. **Type with the badge visible, then open `#signup` and do the same.** Zero renders versus one per keystroke.
2. **Change `mode` to `"onChange"`, then `"onSubmit"`.** One word, and the form's personality changes — §8.5's strategy table as a config option.
3. **Try `{...register("firstName")}` on your `TextField`.** It fails. **Remember this** — it's exactly what `Controller` exists to solve in §8.12.
4. **Add `confirmPassword`.** You'll need `validate` + `getValues("password")`, plus `deps: ["confirmPassword"]` on the password field or the rule goes stale. Then compare with §8.6 experiment 1, where the same rule was one `.refine()`. That contrast is the argument for the next step.
5. **Log `formState`** on each render. You'll see how much the library is tracking for you — that inventory is what you'd otherwise be writing.

---

## 8.9 react-hook-form + zod

Step 8.6 replaced the rules with a schema. Step 8.8 replaced the state management. Those were **independent changes**, and this step does both at once — which takes three lines.
...
import { SignUpSchema, type SignUpValues } from "@/demos/08-forms/signupSchema"

const { register, handleSubmit, /* … */ } = useForm<SignUpValues>({
  resolver: zodResolver(SignUpSchema),        // 1. add the resolver
  mode: "onTouched",
  defaultValues: emptySignUp,
})

// 2. the rules disappear from register
<Form.Control {...register("firstName")} isInvalid={Boolean(errors.firstName)} />

// 3. cross-field rules come free, from the schema's .refine()
```

**The schema file is unchanged from §8.6.** That's the headline: it doesn't know or care whether your state lives in `useState` or in react-hook-form. Only the line that *runs* it differs — `SignUpSchema.safeParse(values)` there, `zodResolver(SignUpSchema)` here.

**What the resolver buys over `register`'s rules:**

| | `register` rules | zod schema |
|---|---|---|
| Cross-field rules | `validate` + `getValues` + `deps` | `.refine()` with `path` |
| Produces the TypeScript type | ✗ declared separately | ✅ `z.infer` |
| Testable without rendering | ✗ | ✅ |
| Reusable on a server | ✗ | ✅ |
| Coercion (`"42"` → `42`) | ✗ | ✅ `z.coerce.number()` |
| Rules visible in one place | ✗ scattered through JSX | ✅ one block |
| Readable for 3 trivial fields | ✅ | slightly more setup |

---

## 🧪 Lab 8.9 — SignUp with react-hook-form + zod
...
**1. Add the cross-field rule to `signupSchema.ts`** — the thing `register` made awkward:
...
export const SignUpSchema = z
  ...
  .refine((data) => data.password === data.confirmPassword, {
    message: "The two passwords don't match.",
    path: ["confirmPassword"],        // ← which field shows the message
  })
...
**2. Create `src/demos/08-forms/SignupRHFZod.tsx`** — `SignupRHF.tsx` with the rules removed:
...
function SignupRHFZod() {
  const {
    ...
    resolver: zodResolver(SignUpSchema),      // ← the only structural change
    mode: "onTouched",
    defaultValues: emptySignUp,
  })
  ...
  return (
    <>
      ...
      <Form onSubmit={handleSubmit(onValid, onInvalid)} noValidate>
        <Form.Group className="mb-3" controlId="signUpRHFZod.fname">
          <Form.Label>First Name</Form.Label>
          <Form.Control
            placeholder="Enter Your First Name"
            isInvalid={Boolean(errors.firstName)}
            {...register("firstName")}        {/* ← no rules here any more */}
          />
          <Form.Control.Feedback type="invalid">
            {errors.firstName?.message}
          </Form.Control.Feedback>
        ...
        <Form.Group className="mb-3" controlId="signUpRHFZod.lname">
          <Form.Label>Last Name</Form.Label>
          <Form.Control
            placeholder="Enter Your Last Name"
            isInvalid={Boolean(errors.lastName)}
        ...
        <Form.Group className="mb-3" controlId="signUpRHFZod.username">
          <Form.Label>Username</Form.Label>
          <Form.Control
            type="email"
            placeholder="Enter Your Email — try someone@taken.com"
        ...
        <Form.Group className="mb-3" controlId="signUpRHFZod.password">
          <Form.Label>Password</Form.Label>
          <Form.Control
            type="password"
            isInvalid={Boolean(errors.password)}
        ...
        <Form.Group className="mb-3" controlId="signUpRHFZod.confirmPassword">
...
Create `src/demos/08-forms/SignupRHFZodLab.tsx`:

```tsx
import { Card } from "react-bootstrap"
import DemoCard from "@/lab/DemoCard"
import RenderBadge from "@/lab/RenderBadge"
import SignupRHFZod from "@/demos/08-forms/SignupRHFZod"

export default function SignupRHFZodLab() {
  return (
    <DemoCard
      title="Signup with react-hook-form + zod"
      claim="The two axes are independent: §8.6 changed the rules, §8.8 changed the state, and this step does both with one resolver line."
      level="core"
      notice={
        <ul className="mb-0">
          <li>
            Every <code>register()</code> call is now bare — all the rules live in one
            readable block in <code>signupSchema.ts</code>.
          </li>
          <li>
            Type mismatched passwords, then fix the <em>first</em> one. The error under
            Confirm clears by itself. §8.8's version needed <code>deps</code> to manage
            that; <code>.refine()</code> needs nothing.
          </li>
          <li>
            The schema file is byte-for-byte the one from §8.6 — it doesn't know whether
            your state lives in <code>useState</code> or in the library.
          </li>
        </ul>
      }
    >
...
```tsx
{ id: "signup-rhf-zod", chapter: "8 — Forms", title: "Signup with RHF + zod", element: <SignupRHFZodLab /> },
```

### What to notice

- **The JSX lost every rule.** `{...register("firstName")}` with no second argument. All the rules are in one readable block in `signupSchema.ts`.
- **`confirmPassword` needed no `validate`, no `getValues`, no `deps`.** The schema's `.refine()` runs on every validation, so changing the *password* updates `confirmPassword`'s error automatically — the stale-result bug from §8.8 simply can't occur.
- **`SignUpValues` is inferred.** Add a field to the schema and `defaultValues` stops compiling until you add it there too.
- **`onValid` receives typed, validated data.** It's the only path to your API.

### Experiments

1. **Delete the `resolver` line.** All validation vanishes and `onValid` fires with anything. The resolver is the *only* connection between schema and form — swap it for Yup or Valibot without touching a field.
2. **Type mismatched passwords, then fix the first one.** The error under `confirmPassword` clears by itself. Now go back to §8.8's version and try the same: without `deps`, it doesn't.
3. **Move `path: ["confirmPassword"]` to `["password"]`.** The message jumps fields. `register` had no way to express this at all.
4. **Add a field to the schema but not to `defaultValues`.** TypeScript errors immediately, because the type is derived.
5. **Write a test for the rules** — `SignUpSchema.safeParse({...})` in a `.test.ts`, no renderer. Then try to test §8.8's `register` rules the same way. You can't; they only exist inside a rendered form.

---

## 8.10 Every input type, read raw
...
URL parameter, or `localStorage`. Step 8.11 makes this automatic — a generic `SelectField<T>` puts the
cast inside the component, once.

## 🧪 Lab 8.10 — Every input kind, correctly typed

**Level:** core

A reference you'll come back to. Each field shows the property you read and the type you get — raw, with no wrapper. Step 8.11 turns every one of these into a reusable component.
...
## 8.11 A component for every input type
...
## 🧪 Lab 8.11 — A field component for every input type

**Level:** core — the component library §8.12 is built on

The sign-up form works, validates, and runs on react-hook-form — and every field in it is text. Real forms aren't: they have selects, radios, checkboxes, dates, files and ranges. Step 8.10 showed *which property to read* for each; this lab wraps every one in the same controlled contract you gave `TextField` in §8.3. Step 8.12 then drives the whole set from react-hook-form.
...
> **delete `TextField.tsx`** and repoint the two imports that used it, in `Signup.tsx` and
> `SignupZod.tsx`:
>
> ```diff
> - import TextField from "@/demos/08-forms/TextField"
...
2. **Try `set("role", "owner")`.** Compile error — `SelectField<Role>` inferred `T` from the options, so the parent's union is preserved end to end with no cast at the call site. Compare with the raw version in Lab 8.10, where every `<Form.Select>` needed `as Role`.
...
5. **Pass `error="Something's wrong"` to any field.** It renders correctly without the component knowing anything about validation — which is what makes §8.12 possible.

---

---

## 8.12 The full form: every field type with react-hook-form

This step answers the question §8.8 left hanging. `register` couldn't drive our `TextField`, so
§8.8 and §8.9 went back to hand-written `Form.Group` blocks. Now we have eleven field components and
a form library — can we have both?
...
`field` carries `{ value, onChange, onBlur, name, ref }` — **the exact contract you gave `TextField`
back in §8.3.** That's not luck. Designing components as controlled components, reporting values
rather than events, is what makes them drop into any form library later.

**The rule:** `register` for anything that renders a real DOM input (react-bootstrap's
`Form.Control` forwards refs, so the spread reaches the `<input>`); `Controller` for your own
components, third-party date pickers, and anything with a non-DOM `onChange`.

**One honest caveat.** `Controller` makes that field controlled again, so the render-count win from
§8.8 is smaller here — you're trading some of it back for reusable components. Whether that's worth
it depends on your form; at eleven field types and fifteen fields, it is.

## 🧪 Lab 8.12 — The full form: every field type with react-hook-form

**Level:** core

> This is the last step, and the one that answers the question §8.8 left open:
> *can we have the library's mechanics and our own field components?* Yes — via `Controller`.

The payoff. Every field type from §8.11, driven by react-hook-form through `Controller`, validated by one zod schema — and not one field component needed changing to get here.

```bash
npm install react-hook-form @hookform/resolvers
```

### Step 1 — the schema and the option lists

Lab 8.7's `signupSchema.ts` covered five text fields. This form has fifteen of every kind, so it gets
its own schema. Create `src/demos/08-forms/fullSignupSchema.ts`:

```ts
import { z } from "zod"

// The option lists live here too, so the schema and the <SelectField> options
...
export const FullSignUpSchema = z
  .object({
    ...
    // password pair — the cross-field rule is at the bottom
    password: z
      .string()
      .min(8, "Use at least 8 characters.")
      .regex(/\d/, "Include at least one number."),
    ...
    // number, coerced from the input's string
    age: z.coerce
    ...
    // enums — the values come from the lists above
    role: z.enum(["viewer", "editor", "admin"]),
    plan: z.enum(["free", "pro", "team"]),

    // an array with a minimum
    interests: z
      .array(z.enum(["react", "typescript", "testing", "design"]))
      .min(1, "Pick at least one interest."),

    // free text with a cap
    bio: z.string().max(140, "Keep it under 140 characters."),

    // range
    experience: z.number().min(0).max(20),

    // booleans — note the difference between the two
...
Every field type from Lab 8.9 now has a matching rule: `z.enum` for the select and radios,
`z.array(...).min(1)` for the checkbox group, `z.coerce.number()` for the number input,
`z.literal(true)` for the terms box (`z.boolean()` would happily accept `false`), and `.refine()`
for the pair of passwords.

### Step 2 — the form

Create `src/demos/08-forms/SignUpFullFormLab.tsx`:
...
import RenderBadge from "@/lab/RenderBadge"
import {
  TextField, NumberField, TextAreaField, SelectField, RadioGroupField,
  CheckboxGroupField, CheckboxField, DateField, RangeField,
} from "@/demos/08-forms/fields"
import {
  FullSignUpSchema, ROLES, PLANS, INTERESTS, type FullSignUpValues,
} from "@/demos/08-forms/fullSignupSchema"

export default function SignUpFullFormLab() {
  const [saved, setSaved] = useState<FullSignUpValues | null>(null)
  ...
  const {
    ...
    resolver: zodResolver(FullSignUpSchema),   // the full-form schema from Step 1
    mode: "onTouched",                     // §8.5's strategy, as one option
    defaultValues: {
      firstName: "", lastName: "", username: "", email: "",
      password: "", confirmPassword: "", age: 18, startDate: "",
      role: "viewer", plan: "free", interests: [], bio: "",
      experience: 3, newsletter: true,
      terms: false as unknown as true,     // literal(true) needs a false start
    },
  })

  async function onValid(values: FullSignUpValues) {
    await new Promise((r) => setTimeout(r, 700))
  ...
  function onInvalid(formErrors: FieldErrors<FullSignUpValues>) {
    console.warn("❌ Submit blocked")
    console.table(
      Object.fromEntries(
        Object.entries(formErrors).map(([k, v]) => [k, (v as { message?: string })?.message])
      )
    )
  }
  ...
  return (
    <DemoCard
      title="SignUp with react-hook-form + zod"
      claim="The library owns registration, touched tracking, the submit lifecycle and error placement. You keep the schema and the field components — and typing stops re-rendering the form."
      level="core"
      notice={
        <ul className="mb-0">
          <li>
            <strong>Type in any field and watch the render badge.</strong> It barely moves.
            Compare with Lab 8.1, where every character caused a render.
          </li>
          <li>
            Compare with §8.5: gone are <code>draft</code> state, the <code>set</code>{" "}
            helper, the <code>touched</code> map, <code>submitAttempted</code>,{" "}
            <code>errorFor</code>, and the manual <code>safeParse</code>.{" "}
            <strong>The schema and the JSX are what's left.</strong>
          </li>
          <li>
            <code>handleSubmit(onValid, onInvalid)</code> takes <strong>two</strong>{" "}
            callbacks — exactly the two branches you wrote by hand in §8.8.
          </li>
          <li>
            Every field goes through <code>&lt;Controller&gt;</code>, because our components
            take <code>onChange(value)</code> rather than a DOM event.{" "}
            <code>field</code> hands over <code>{"{ value, onChange, onBlur }"}</code> —{" "}
            <strong>the exact contract we chose in §8.3.</strong> That's not luck:
            designing components as controlled components is what makes them drop into any
            form library.
          </li>
          <li>
            Submit with an email ending <code>@taken.com</code> to see{" "}
    ...
    >
      <Row className="g-3">
        <Col lg={7}>
          <Card body>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <span className="small fw-semibold text-muted">Sign up</span>
              <RenderBadge label="form renders" bg="success" />
            </div>
            ...
            <Form onSubmit={handleSubmit(onValid, onInvalid)} noValidate>
              <Row className="g-0">
                <Col sm={6} className="pe-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <TextField
                        controlId="fullForm.firstName" label="First Name"
                ...
                <Col sm={6} className="ps-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <TextField
                        controlId="fullForm.lastName" label="Last Name"
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                      />
                    )}
                ...
                <Col sm={7} className="pe-sm-2">
                  <Controller
                    name="username" control={control}
                    render={({ field, fieldState }) => (
                      <TextField
                        controlId="fullForm.username" label="Username"
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                ...
                <Col sm={5} className="ps-sm-2">
                  <Controller
                    name="age" control={control}
                    render={({ field, fieldState }) => (
                      <NumberField
                        controlId="fullForm.age" label="Age" min={0} max={120}
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                        hint="a real number — z.coerce is a no-op here"
                      />
                    )}
                  />
                </Col>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <TextField
                    controlId="fullForm.email" label="Email" type="email"
                    value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                    error={fieldState.error?.message}
              ...
              <Row className="g-0">
                <Col sm={6} className="pe-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <TextField
                        controlId="fullForm.password" label="Password" type="password"
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                      />
                    )}
                ...
                <Col sm={6} className="ps-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <TextField
                        controlId="fullForm.confirmPassword" label="Confirm password" type="password"
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                      />
                    )}
                ...
                <Col sm={6} className="pe-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <SelectField
                        controlId="fullForm.role" label="Role" options={ROLES}
                ...
                <Col sm={6} className="ps-sm-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <DateField
                        controlId="fullForm.startDate" label="Start date"
                        value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                        error={fieldState.error?.message}
                      />
                    )}
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <RadioGroupField
                    controlId="fullForm.plan" label="Plan" options={PLANS}
                    value={field.value} onChange={field.onChange}
                    error={fieldState.error?.message}
                  />
                )}
              ...
              <Controller
                name="interests" control={control}
                render={({ field, fieldState }) => (
                  <CheckboxGroupField
                    controlId="fullForm.interests" label="Interests" options={INTERESTS}
                    value={field.value} onChange={field.onChange}
                    error={fieldState.error?.message}
                  />
                )}
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <TextAreaField
                    controlId="fullForm.bio" label="Bio" maxLength={140}
                    value={field.value} onChange={field.onChange} onBlur={field.onBlur}
                    error={fieldState.error?.message}
                  />
                )}
              ...
              <Controller
                name="experience" control={control}
                render={({ field }) => (
                  <RangeField
                    controlId="fullForm.experience" label="Years of experience"
                    min={0} max={20} value={field.value} onChange={field.onChange}
                    format={(n) => `${n} yr${n === 1 ? "" : "s"}`}
                  />
                )}
              />
              ...
              <Controller
                name="newsletter" control={control}
                render={({ field }) => (
                  <CheckboxField
                    controlId="fullForm.newsletter" type="switch" label="Send me the newsletter"
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <CheckboxField
                    controlId="fullForm.terms" label="I accept the terms"
                    checked={field.value} onChange={field.onChange}
                    error={fieldState.error?.message}
                  />
                )}
        ...
        <Col lg={5}>
          <Card body className="mb-3">
            <div className="small fw-semibold text-muted mb-2">formState</div>
            <div className="d-flex flex-wrap gap-2">
              <Badge bg={isDirty ? "warning" : "secondary"}>isDirty: {String(isDirty)}</Badge>
              <Badge bg={isValid ? "success" : "secondary"}>isValid: {String(isValid)}</Badge>
              <Badge bg={isSubmitting ? "info" : "secondary"}>
          ...
          <div className="small fw-semibold text-muted mb-2">
            touchedFields — maintained for you
          </div>
          <StateInspector label="touched" value={touchedFields} />

          <div className="small fw-semibold text-muted mt-3 mb-2">errors</div>
          <StateInspector
            label="errors"
          ...
          {saved && (
            <Alert variant="success" className="small mt-3 mb-0">
              <div className="fw-semibold mb-1">Submitted and reset</div>
              <code>{saved.username}</code> · age {saved.age} (a {typeof saved.age}) ·{" "}
              {saved.interests.length} interest(s)
            </Alert>
          )}
        </Col>
      </Row>
...
Register as `{ id: "signup-full-form", chapter: "8 — Forms", title: "Full form with react-hook-form", element: <SignUpFullFormLab /> }`.

**Experiments — the first is the one to actually do:**

1. **Open `#signup`, `#signup-rhf` and this lab side by side and count.** Same form, same rules, same field components. What disappeared each time was state management, then rule-writing, then plumbing. That progression is the entire argument of §8.
2. **Watch the render badge while typing**, then compare with Lab 8.1's. Note the honest caveat: because every field here uses `Controller`, RHF *does* re-render the controlled subtree — the saving is real but smaller than a `register`-only form. Swap one `TextField` for a raw `<Form.Control {...register("bio")} />` and watch that field stop re-rendering entirely.
3. **Change `mode` to `"onChange"`, then `"onSubmit"`.** One word, and the form's personality changes — §8.8's strategy table as a config option.
4. **Remove `resolver: zodResolver(FullSignUpSchema)`.** All validation disappears and `onValid` fires with anything. The resolver is the only link between schema and form — swap it for Yup or Valibot without touching a field.
5. **Try `{...register("firstName")}` on our `TextField`.** It fails: `register` spreads a `ref` and a DOM-shaped `onChange`, and `TextField` accepts neither. **That's the rule — `register` for raw `Form.Control`s, `Controller` for your own components.**
...
Four combinations, all four shipped in real codebases, and you've built all four:

|  | **Rules written by hand** | **Rules in a zod schema** |
|---|---|---|
| **State in `useState`** | `Signup.tsx` — §8.5 | `SignupZod.tsx` — §8.6 |
| **State in react-hook-form** | `SignupRHF.tsx` — §8.8 | `SignupRHFZod.tsx` — §8.9 |

**Reading across a row** shows what zod changed: the type stops being declared and starts being *derived*, the cross-field rule collapses to one `.refine()`, and the rules become testable without a renderer and reusable on a server. Nothing about state management moves.

**Reading down a column** shows what react-hook-form changed: `values`, `update`, `touched`, `submitAttempted`, `errorFor` and the derived `errors` all disappear, and so does one re-render per keystroke. What it costs is the reusable `TextField` — `register` spreads a `ref` and a DOM-shaped `onChange`, which `TextField` doesn't accept, so §8.8 and §8.9 write a `Form.Group` per field by hand.

Most tutorials show only the top-left and bottom-right, which makes this look like one big all-or-nothing decision. It isn't.

| Situation | Reach for |
|---|---|
| One or two fields, no rules | A controlled input (§8.1) |
| A field used more than once | Extract a stateless field component (§8.2–8.3) |
| Values only needed at submit | Uncontrolled + `FormData` (§8.7) |
...
| Rules that also guard an API response or `localStorage` | A zod schema (§8.6) |
| Many fields, or a real submit lifecycle | react-hook-form (§8.8) |
| Both of the last two | react-hook-form + zod (§8.9) |
| Any of those, but you want your own field components back | + `Controller` (§8.12) |

**The cheapest useful upgrade on that list is §8.6** — adding zod to a form you already control costs one dependency and changes one function. No new mental model, no uncontrolled inputs, no `Controller`. If you're unsure where to start, start there.

**Don't reach for §8.9 on a search box.** A single controlled input is three lines; `useForm` is two dependencies, a schema and a resolver. Match the tool to the form.
...
**Verify:** add a task and it appears at the top of the list with the priority you chose. Submit an empty title and the red feedback appears, then clears as you type.

Four things worth noticing, because each is a deliberate choice you'll re-make in every form you write:

1. **`isInvalid` + `Form.Control.Feedback`** is Bootstrap's built-in validation display — no custom error markup, and it sets `aria-invalid` for you.
2. **`controlId` on `Form.Group`** wires the label to the input automatically, so you never write matching `htmlFor` and `id`.
3. **The form state lives *inside* `AddTaskForm`** while the task list lives in `App`. Form state is local; only the finished result travels upward, as a `NewTask`. **Keep state as low as it can go** — §9 argues this properly.
4. **The parent constructs the `Task`, not the form.** The form knows about `NewTask` (title and priority) and nothing about ids, timestamps, or `done`. That boundary is why the same form could later be reused for editing.

**Try it yourself:** the error clears on the next keystroke because of the `if (error) setError("")` inside `onChange`. Remove it and note how much more irritating the form becomes. Then compare with Lab 8.4's derived-errors approach and consider which you'd rather maintain — TaskBoard's version is simpler, and the lab's version scales.
...
5. **`as Task[]`** because `JSON.parse` returns `any`. This is an honest limitation: TypeScript cannot verify data from outside your program. The cast documents an assumption you're choosing to make. In production, validate the parsed shape with zod instead of asserting it — §17 shows how.

That last point deserves a moment. The cast means a `taskboard.tasks` entry containing `[{"title": 42}]` — from an older version of your app, or a user with the console open — flows straight into your typed state and crashes somewhere far away. `as` is a promise you make to the compiler, and the compiler has no way to check it.
...
function init(fallback: Task[]): Task[] {
  try {
    const saved = localStorage.getItem(STORAGE_KEY)
    return saved !== null ? (JSON.parse(saved) as Task[]) : fallback
  } catch {
    return fallback
  }
}
...
export default function EditTaskModal({ task, show, onHide }: EditTaskModalProps) {
  ...
  function handleSave() {
    ...
    update(task.id, { title: trimmed, priority })
    onHide()
  }
...
**Verify:** click a pencil icon, the modal opens with the task's current values and the input focused. Change the title, press Enter, and the card updates.

Five things to notice, each of which draws on an earlier section:

- **`onEntered`** is React-Bootstrap's callback for "the enter transition has finished". Focusing there is more reliable than a `useEffect` with a `setTimeout`, because the modal is genuinely mounted and visible by then. **Reach for the library's own lifecycle hooks before improvising** — this is the same lesson as `controlId` in Build Step 5.
- **`show={editing !== null}`** is **derived** from `editing` rather than being separate state (§10). One value, no chance of the two disagreeing.
...
- **The draft-syncing effect is the one case §11.4's table calls "usually no".** It's here because the modal stays mounted between edits, so there's no unmount to reset it. The tidier alternative is §4.3's key trick, which the exercise below asks you to try.
- **The modal reaches the store via `useTaskActions`** but takes `task` and `onHide` as props, because those are `App`'s view state (§14's closing note).

**Try it yourself:**

1. Replace the syncing effect with a `key`. Move the modal to `<EditTaskModal key={editing?.id ?? "none"} … />` and initialise the state directly from the prop: `useState(task?.title ?? "")`. The effect disappears, there's one render instead of two, and the "reset when the selection changes" logic becomes a single attribute. This is §4.3 and §11.4 combining into a genuinely better design.
2. Open the modal and press Escape. React-Bootstrap calls `onHide` for you — another library lifecycle you didn't have to build.
3. Open a task, edit the title without saving, close, and open a *different* task. With the effect version, does the draft leak? With the key version? Try both.
...
Two levels: how it works by hand, and what you'd actually use in production. Learn the first so you can debug the second.
...
## 🔨 Build Step 13 — Routing and URL state in TaskBoard
...
## 🔨 Build Step 14 — Boundaries in TaskBoard
...
## 🔨 Build Step 15 — Tests for TaskBoard
...
**What you've got for about 100 lines:** every state transition in the reducer verified, the immutability convention locked in, the form's validation and reset behaviour pinned, and both empty states protected. That's the core logic of the app.
...
| 8.6 Swap the validator for a schema | core | Three lines change — zod is a validation decision, not architectural | `08-forms/SignupZodLab.tsx` |
| 8.7 Controlled vs uncontrolled | core | The design react-hook-form is built on | `08-forms/ControlledLab.tsx` |
| 8.8 SignUp with react-hook-form | core | `register`, `formState`, no re-render per keystroke | `08-forms/SignupRHFLab.tsx` |
| 8.9 SignUp with RHF + zod | core | The two axes are independent; `.refine()` beats `deps` | `08-forms/SignupRHFZodLab.tsx` |
| 8.10 Every input kind (raw) | core | Which property to read for each input type | `08-forms/InputKindsLab.tsx` |
| 8.11 A component for every type | core | The same contract, eleven times | `08-forms/AllFieldsLab.tsx` |
| 8.12 The full form with RHF | core | `Controller` vs `register`, and why our contract fits | `08-forms/SignUpFullFormLab.tsx` |
...
import SingleInputLab from "@/demos/08-forms/SingleInputLab"
import SignupLab from "@/demos/08-forms/SignupLab"
import SignupZodLab from "@/demos/08-forms/SignupZodLab"
import ControlledLab from "@/demos/08-forms/ControlledLab"
import SignupRHFLab from "@/demos/08-forms/SignupRHFLab"
import SignupRHFZodLab from "@/demos/08-forms/SignupRHFZodLab"
import InputKindsLab from "@/demos/08-forms/InputKindsLab"
import AllFieldsLab from "@/demos/08-forms/AllFieldsLab"
import SignUpFullFormLab from "@/demos/08-forms/SignUpFullFormLab"
...
export const demos: Demo[] = [
  ...
  { id: "signup-rhf", chapter: "8 — Forms", title: "Signup with react-hook-form", element: <SignupRHFLab /> },
  { id: "signup-rhf-zod", chapter: "8 — Forms", title: "Signup with RHF + zod", element: <SignupRHFZodLab /> },
  ...
  { id: "signup-full-form", chapter: "8 — Forms", title: "Full form with react-hook-form", element: <SignUpFullFormLab /> },
...
| 5 | Add-task form | Controlled inputs, validation, `controlId`, state colocation |
| 6 | Filters, search, stats | Derived state, two empty states, union-typed props |
| 7 | `localStorage` persistence | Effects, lazy init, `try`/`catch`, cast honesty |
...
| 10 | Edit modal with autofocus | Refs, library lifecycle hooks, derived `show` |
| 11 | Profile it (no code changes) | The Profiler, and why *not* to memoise |
| 12 | Load seed tasks from an API | Fetch, abort, zod validation, union load state |
| 13 | Routing and URL state | Router, `Outlet`, `useSearchParams`, store-the-id |
| 14 | Layered error boundaries | Error boundaries, containment granularity |
| 15 | Tests | Reducer tests, behaviour tests, accessible queries |
