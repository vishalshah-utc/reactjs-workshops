    ...
    children: [
...

      {
        ...
        children: [
          ...
          {
            // The console is NOT a loader route: the store owns the async.
            // requireRole('admin') is still the lock, exactly as on `team`.
            path: 'inventory',
            middleware: [requireRole('admin')],
            lazy: async () => {
              const { InventoryPage } = await import('./routes/account/InventoryPage');
              const { inventoryLoader } = await import('./routes/account/inventoryLoader');
              return { Component: InventoryPage, loader: inventoryLoader };
            },
          },
          {
            path: 'inventory/rtkq',
            middleware: [requireRole('admin')],
            lazy: async () => {
              const { InventoryQueryPage } = await import('./routes/account/InventoryQueryPage');
              const { inventoryLoader } = await import('./routes/account/inventoryLoader');
              return { Component: InventoryQueryPage, loader: inventoryLoader };
            },
          },
          {
            path: 'team',
