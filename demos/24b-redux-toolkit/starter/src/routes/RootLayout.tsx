...
export function RootLayout() {
  ...
  function handleSignOut() {
    ...
    // TODO(lab-5.4): dispatch ONE `signedOut()` action here and let the
    // inventory and recent slices each decide what it means for them — the
    // cart and wishlist, which are Zustand and not user-scoped, stay as they
    // are. The RTK Query cache is separate state with its own reset
    // (`inventoryApi.util.resetApiState()`), because it holds somebody else's
    // data, not ours.
    navigate('/products');
  }
