 ...
 *
 * `select=reviews` asks DummyJSON for that field and nothing else, so this is
 * a small response — and, more to the point, a SEPARATE one. Demo 21 Block 2
 * needs a section of the page that arrives later than the rest, and the only
 * honest way to have one is to fetch it separately.
 *
 * `delay` is DummyJSON's own parameter: it holds the response open for that
 * many milliseconds. It is a teaching switch, not production code — which is
 * why it defaults to 0 and the caller has to ask for it.
 */
