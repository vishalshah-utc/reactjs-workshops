...
import { ADMIN_USER, CATALOGUE, reviewsFor } from '../test/fixtures';
...
describe('productDetailLoader', () => {
  ...
  /**
   * The Demo 21 loader returns `{ product, reviews }` where `reviews` is a
   * promise it never awaited. These two tests are the contract that buys:
   * the page does not wait for the slow half, and the slow half failing does
  ...
  it('renders the product while the reviews are still loading, then streams them in', async () => {
    ...
    // …and the fallback is replaced in place, with nothing above it re-mounting.
  ...
  it('sends a rejected reviews promise to the widget boundary, not the page', async () => {
    ...
    // `use()` re-throws the rejection during render. The boundary INSIDE the
    // tab panel catches it, so the product is still on screen.
