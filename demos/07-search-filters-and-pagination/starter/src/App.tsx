...
export default function App() {
  ...
  // TODO(lab-1.3): debounce the query; send q/category/sort to the SERVER; one `filters` object + updateFilters(patch); delete filterProducts/applySort
  // TODO(lab-2.1): a SECOND effect that loads categories from the API, with its own AbortController
  // TODO(lab-3.1): add `page` to filters — updateFilters resets it to 0 on any other change; pass page + PAGE_SIZE to listProducts
  // TODO(lab-4.3): selectedId state (number | null); render <ProductDetail id={selectedId} onClose=… />
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const data = await listProducts({ signal: controller.signal }); // limit defaults to 0 → all 194
        setProducts(data.products);
      } catch (err) {
        if (axios.isCancel(err)) return; // a cancelled request is not a failure
        // The interceptor already made this an ApiError at runtime; `from()` is
        // idempotent and tells TypeScript so — `unknown` in, ApiError out.
