...
export function ProductCard({ product, density = 'comfortable', saved = false, onToggleSave, onAddToCart, onEdit, onDelete, busy = false }: ProductCardProps) {
  ...
  return (
    // TODO(lab-2.1): the three template-string classNames on this card become clsx(...) — no `${cond ? 'x' : ''}`.
    // TODO(lab-4.3): add our own class names for the container query: 'product-card' on the Card, 'product-card__media' on this Link.
