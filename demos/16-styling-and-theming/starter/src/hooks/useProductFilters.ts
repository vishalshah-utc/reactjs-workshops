...
export function useProductFilters() {
  ...
  const updateFilters = useCallback(
    (
      ...
      // preventScrollReset: the caller is about to scroll somewhere itself (the Pager) — don't let ScrollRestoration jump to the top first.
      { replace = true, preventScrollReset = false }: { replace?: boolean; preventScrollReset?: boolean } = {},
    ) => {
      setSearchParams(
        (previous) => {
