...
export function ProductsPage() {
  ...
  // TODO(lab-3.4): const gridRef = useRef<HTMLDivElement>(null) — hand it to ProductGrid as `ref` and to Pager as `scrollTargetRef`; page changes pass preventScrollReset
  // TODO(lab-4.3): showChart state + chartPanelRef; a "Prices" disclosure button (aria-expanded/aria-controls) in the header actions; flushSync(() => setShowChart(true)) then scrollIntoView; render <PriceHistogram products={products} /> above the grid
  // TODO(lab-5.3): that scrollIntoView uses prefersReducedMotion() ? 'auto' : 'smooth'
  ...
  // TODO(lab-1.3): a "last reported search" ref + effect — logger.debug once per DISTINCT committed query, never on a re-render. Compare `lastFlash` below.
