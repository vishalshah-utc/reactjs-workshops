...
import { epicMiddleware, runEpics } from './epics';
...
export const store = configureStore({
  ...
  middleware: (getDefaultMiddleware) =>
    getDefaultMiddleware({
      ...
      /**
       * ORDER, and it is different from Demo 24b's.
       *
       * The listener middleware was PREPENDED, so a listener could see an
       * action before a thunk dispatched in response to it. The epic
       * middleware must be CONCATENATED — placed after the thunk middleware —
       * because redux-observable pushes an action into `action$` only after it
       * has reached the reducers, and anything upstream of the thunk
       * middleware would see thunk *functions* travelling down the chain
       * rather than actions.
       *
       * The practical rule: epics see plain actions, after the reducers have
       * already handled them. `state$.value` inside an epic is therefore
