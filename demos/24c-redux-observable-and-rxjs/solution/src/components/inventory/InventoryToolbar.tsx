...
export function InventoryToolbar({ categories }: InventoryToolbarProps) {
  ...
  /**
   * THE DEBOUNCE MOVED, AND SO DID EIGHT LINES OF THIS COMPONENT.
   *
   * Demo 24b kept a local draft here, committed it through
   * `useDebouncedCallback` (a 33-line hook with two refs and a cleanup
   * effect), and then needed an adjust-during-render block to follow the store
   * when Reset or sign-out emptied `filters.q` underneath it. Two sources of
   * truth, reconciled by hand.
   *
   * The debounce is now `debounceTime(400)` in `loadEpic`. So this input can
   * go back to being an ordinary controlled input over one source of truth:
   * every keystroke is an action, and exactly one request goes out.
   *
   * THE TRADE, stated plainly because it is real: the devtools log now shows
   * `filters/searchChanged` once per character instead of once per pause. You
   * have moved noise from the network to the recording. On balance that is the
   * better place for it — the recording is a development tool and the network
   * is the user's — but it is a trade, not a free win, and the guide says so.
   */
