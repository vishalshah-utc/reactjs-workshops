...
export function InventoryPage() {
  ...
  /**
   * TODO(lab-1.5): announce the lifecycle, and nothing else.
   *
   *   useEffect(() => {
   *     dispatch(consoleOpened());
   *     return () => { dispatch(consoleClosed()); };
   *   }, [dispatch]);
   *
   * Compare it with what this effect used to be, three lines up in the
   * comment above. It did not disappear — be honest about that — but it
   * stopped knowing anything: no filters in the dependency array, no promise,
   * no abort, no idea that a request exists.
   */
