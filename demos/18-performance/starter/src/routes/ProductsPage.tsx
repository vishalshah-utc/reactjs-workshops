...
export function ProductsPage() {
  ...
  // TODO(lab-1.4): a `slow` useState (NOT a search param — flipping it must not refetch), passed to
  //   ProductToolbar's dev-only switch and down to ProductGrid; and `<RenderProfiler id="grid">` around the grid.
  // TODO(lab-2.1): delete `queryDraft`, `commitQuery`, `seenQuery` and `handleQueryChange` — they move into
  //   ProductToolbar — and pass `query={filters.query} onQueryCommit={(query) => updateFilters({ query })}`.
  // TODO(lab-2.3): render through `<ProductsSurface>`: `title`, `description`, `products`, `actions` (density,
  //   "Show all", "Add product"), `toolbar` (ProductToolbar + CategoryStrip) and the grid as CHILDREN. Density
  //   stays here — say in a comment why it is neither pushed down nor put in the URL.
  // TODO(lab-3.2): `useCallback` for the handlers you hand each card, so `memo(ProductCard)` can actually bail.
  // TODO(lab-4.2): `quickFilter` state + `useDeferredValue`; filter `products` by the DEFERRED value and dim the
  //   grid with `opacity` while `quickFilter !== deferredFilter`.
  // TODO(lab-4.3): `useTransition` around the density change, with `isPending` driving a Spinner.
  // TODO(lab-5.2): a `showAll` disclosure that mounts `<ProductRowList slow={slow} />`.
  // TODO(lab-6.2): delete Lab 3.2's useCallbacks once the compiler is on, and re-profile.
