...
export function RelatedProducts({ product }: RelatedProductsProps) {
  // TODO(lab-1.4): replace this `useFetch` with `useQuery(relatedQuery(product))`.
  // The JSX barely changes — `request.status` becomes `isPending` / `error`, and
  // `relatedQuery`'s `select` does the filtering that line 23 does by hand.
