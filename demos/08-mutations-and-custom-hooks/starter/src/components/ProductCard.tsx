...
// TODO(lab-2.3): an `onEdit?: (product: Product) => void` prop and a pencil button next to the trash
    ...
    <Card className={`h-100 ${isOutOfStock ? 'opacity-75' : ''}`}>
      ...
      <Card.Body className={`d-flex flex-column gap-2 ${isCompact ? 'p-2' : ''}`}>
        ...
        <Card.Title className={`mb-0 ${isCompact ? 'small text-truncate' : 'fs-6'}`}>
          {/* A real button, not an onClick on a div: keyboard-reachable, announced by screen readers */}
          <button
            type="button"
