...
/**
 * Raw (unzipped) kilobytes, per JavaScript asset. Raw, because that is what the
 * browser has to PARSE.
 *
 * Moved from 680 to 700 in Demo 19, on purpose and with a number behind it.
 * `@tanstack/react-query` added 45.9 kB raw (619.6 → 665.5 kB on the entry
 * chunk) and bought a cache that removes whole requests; the devtools are
 * dev-only and ship nothing. 700 is 665.5 plus the same ~5% headroom the
 * original number carried. A budget you raise every time it fails is not a
 * budget — raise it when you can say what you bought.
 */
const DEFAULT_MAX_KB = 700;
