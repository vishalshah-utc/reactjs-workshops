...
export interface ListProductsOptions extends RequestOptions {
  q?: string;
  category?: string;
  /** Widened today: the Inventory Console sorts by title and stock too. */
  ...
  limit?: number;
  /**
   * DEV ONLY. DummyJSON answers `?delay=2000` two seconds late, which is how
   * Demo 24a makes a race between two requests reproducible on a fast machine.
   * Never send it from production code.
   */
  delayMs?: number;
}
